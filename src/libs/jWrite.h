...
//
// Example:
//		jwOpen( buffer, buflen, JW_OBJECT, JW_PRETTY );		// open
// root node as object 		jwObj_string( "key", "value" );
// jwObj_int( "int", 1 ); 		jwObj_array( "anArray"); 			jwArr_int( 0 ); 			jwArr_int( 1
//); 			jwArr_int( 2 ); 		jwEnd(); 		err= jwClose();
//// close root object
//
// results in:
//
...
// place. If the sequence of calls is incorrect e.g.
//		jwOpen( buffer, buflen, JW_OBJECT, 1 );
//		jwObj_string( "key", "value" );
//			jwArr_int( 0 );
//      ...
//
// then the error code returned from jwClose() would indicate that you attempted
// to put an array element into an object (instead of a key:value pair) To
// locate the error, the supplied buffer has the JSON created upto the error
// point and a call to jwErrorPos() would return the function call at which the
// error occurred
// - in this case 3, the 3rd function call "jwArr_int(0)" is not correct at this
// point.
//
...
//
// To use multiple instances of jWrite, an application has to supply unique
// instances of jWriteControl structures.
//
// This feature is enabled by commenting out the definition of
// JW_GLOBAL_CONTROL_STRUCT
//
// All the jWrite functions then take an additional parameter: a ptr to the
// structure e.g.
...
//
//
#define JW_GLOBAL_CONTROL_STRUCT // <--- comment this out to use applic-supplied
                                 // jWriteControl
