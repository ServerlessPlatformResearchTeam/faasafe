...
void close_proc_fds(trackedprocess_t *tracked_proc) {
  ...
  fprintf(stderr, "All file descriptors closed successfully\n");
}
...
void open_proc_fds(trackedprocess_t *tracked_proc) {
  ...
  if (tracked_proc->clear_fd < 0) {
    fprintf(stderr, "CLEAR_FD FD OPEN ERROR: %s pid:%d\n", strerror(errno),
            tracked_proc->pid);
    close(tracked_proc->clear_fd);
    exit(-1);
  }
  ...
  if (tracked_proc->maps_fd < 0) {
    fprintf(stderr, "MAPS_FD FD OPEN ERROR: %s pid:%d\n", strerror(errno),
            tracked_proc->pid);
    close(tracked_proc->maps_fd);
    exit(-1);
  }
  ...
  if (tracked_proc->pagemap_fd < 0) {
    fprintf(stderr, "PAGEMAP_FD FD OPEN ERROR: %s pid:%d\n", strerror(errno),
            tracked_proc->pid);
    close(tracked_proc->pagemap_fd);
    exit(-1);
  }
  ...
  if (tracked_proc->mem_fd < 0) {
    fprintf(stderr, "MEM_FD FD OPEN ERROR: %s pid:%d\n", strerror(errno),
            tracked_proc->pid);
    close(tracked_proc->mem_fd);
    exit(-1);
  }
  fprintf(stderr, "All file descriptors opened successfully\n");
}
...
int do_task_reset_dirty_track(int fd) {
  ...
  if (ret < 0) {
    if (errno == EINVAL) /* No clear-soft-dirty in kernel */
      ...
      fprintf(stderr, "Can't reset %d's dirty memory tracker (%d): %s\n",
    ...
    for (size_t i = offset; i < (size_t)length; i++) {
      ...
      if (buffer[i] == '\n' && i) {
        ...
        /* Get info about all pages in this page range with pagemap. */
        {
          ...
          fprintf(stderr, "[SANITY] %lx-%lx %d / %ld\n", low, high, count_sd,
                  (high - low) / 4096);
        }
        buffer[y] = '\n';
      }
