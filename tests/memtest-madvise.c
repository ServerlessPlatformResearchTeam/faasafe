...
int main(int argc, char *argv[]) {
  ...
  char *map2_to_merge_p2 =
      mmap((void *)0x7f3638421000, 32 * PSIZE, PROT_READ | PROT_WRITE,
           MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
  assert(!madvise(map2_to_merge_p2, 32 * PSIZE, MADV_HUGEPAGE));
  char *map2_to_merge_p3 =
      mmap((void *)0x7f3638442000, 32 * PSIZE, PROT_READ | PROT_WRITE,
           MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);

  char *map_to_shrink_by_advise =
      mmap((void *)0x7f3639400000, 32 * PSIZE, PROT_READ | PROT_WRITE,
           MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
  ...
  memcpy(map_to_shrink_by_advise, randomblock, 32 * PSIZE);
  checkpoint_me();

  errno = 0;
  printf("Checkpointed, with all randomness\n");
  printf("Asserting map_to_stay is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_stay, 32 * PSIZE));
  printf("Asserting map_to_grow is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_grow, 32 * PSIZE));
  printf("Asserting map_to_shrink is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_shrink, 32 * PSIZE));
  printf("Asserting map_to_stay2 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_stay2, 32 * PSIZE));
  printf("Asserting map_to_merge_p1 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_merge_p1, 32 * PSIZE));
  printf("Asserting map_to_merge_p2 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_merge_p2, 32 * PSIZE));
  printf("Asserting map_to_split is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_split, 65 * PSIZE));
  printf("All initially allocated maps are indeed zeroed\n");
  fflush(stdout);
  printf("Asserting map2_to_merge_p1 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map2_to_merge_p1, 32 * PSIZE));
  printf("Asserting map2_to_merge_p2 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock + 32 * PSIZE, map2_to_merge_p2, 32 * PSIZE));
  printf("Asserting map3_to_merge_p2 is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock + 64 * PSIZE, map2_to_merge_p3, 32 * PSIZE));
  printf("Asserting map_to_shrink_by_advise is not zeroed\n");
  fflush(stdout);
  assert(!memcmp(randomblock, map_to_shrink_by_advise, 32 * PSIZE));
  ...
  //	memset (map_to_shrink_by_advise, 11, 12*PSIZE);
  printf("Successfully set to 1\n");
  fflush(stdout);

  printf("will grow map_to_grow %p \n", map_to_grow);
  map_to_grow = (char *)mremap((void *)map_to_grow, 32 * PSIZE, 64 * PSIZE, 0);
  // map_to_grow = (char*) mremap((void*) map_to_grow, 32*PSIZE, 64*PSIZE,
  // MREMAP_FIXED | MREMAP_MAYMOVE, (void*) map_to_grow);
  ...
  // map_to_shrink = (char*)mremap((void*) map_to_shrink, 32*PSIZE, 16*PSIZE,
  // MREMAP_FIXED | MREMAP_MAYMOVE, (void*) map_to_shrink);
  ...
  // map_to_merge_p1 = (char*) mremap((void*) map_to_merge_p1, 32*PSIZE,
  // 65*PSIZE, MREMAP_FIXED | MREMAP_MAYMOVE, (void*) map_to_merge_p1);
  assert(map_to_merge_p1 != NULL);
  printf("map_to_merge_p1 merged\n");
  fflush(stdout);
  map_to_split =
      (char *)mremap((void *)map_to_split, 65 * PSIZE, 32 * PSIZE, 0);
  // map_to_split =  (char*) mremap((void*) map_to_split, 65*PSIZE, 32*PSIZE,
  // MREMAP_FIXED | MREMAP_MAYMOVE, (void*) map_to_split);
  ...
  printf("madvising map_to_shrink_by_advise\n");
  fflush(stdout);
  madvise((void *)map_to_shrink_by_advise, 10 * PSIZE, MADV_DONTNEED);
  sleep(2);
  dump_stats_me();
  restore_me();
