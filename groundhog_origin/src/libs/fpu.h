/*
 * https://github.com/checkpoint-restore/criu/blob/criu-dev/compel/arch/x86/src/lib/include/uapi/asm/fpu.h
 */
...
#define XFEATURE_MASK_AVX512                                                   \
  (XFEATURE_MASK_OPMASK | XFEATURE_MASK_ZMM_Hi256 | XFEATURE_MASK_Hi16_ZMM)
...
} __attribute__((aligned(FXSAVE_ALIGN_BYTES)));
 ...
 */
#define EXTENDED_STATE_AREA_SIZE                                               \
  (4096 - sizeof(struct i387_fxsave_struct) - sizeof(struct xsave_hdr_struct))
...
} __attribute__((aligned(FP_MIN_ALIGN_BYTES))) __attribute__((packed));
...
} __attribute__((aligned(FXSAVE_ALIGN_BYTES))) fpu_state_ia32_t;
